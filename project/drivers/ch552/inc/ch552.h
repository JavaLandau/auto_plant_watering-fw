...
#include <compiler.h>
#include "ch552_driver_conf.h"

/*----- SFR --------------------------------------------------------------*/
/*  sbit are bit addressable, others are byte addressable */

/*  System Registers  */
#define BYTE_PSW          0xD0
SFR(PSW, BYTE_PSW);                 // program status word
SBIT(CY, BYTE_PSW, 7);              // carry flag
SBIT(AC, BYTE_PSW, 6);              // auxiliary carry flag
SBIT(F0, BYTE_PSW, 5);              // bit addressable general purpose flag 0
SBIT(RS1, BYTE_PSW, 4);             // register R0-R7 bank selection high bit
SBIT(RS0, BYTE_PSW, 3);             // register R0-R7 bank selection low bit
...
SBIT(OV, BYTE_PSW, 2);              // overflow flag
SBIT(F1, BYTE_PSW, 1);              // bit addressable general purpose flag 1
SBIT(P, BYTE_PSW, 0);               // ReadOnly: parity flag
...
/*  Interrupt Registers  */
#define BYTE_IE           0xA8
SFR(IE, BYTE_IE);                   // interrupt enable
SBIT(EA, BYTE_IE, 7);               // enable global interrupts: 0=disable, 1=enable if E_DIS=0
SBIT(E_DIS, BYTE_IE, 6);            // disable global interrupts, intend to inhibit interrupt during some flash-ROM operation: 0=enable if EA=1, 1=disable
SBIT(ET2, BYTE_IE, 5);              // enable timer2 interrupt
SBIT(ES, BYTE_IE, 4);               // enable UART0 interrupt
SBIT(ET1, BYTE_IE, 3);              // enable timer1 interrupt
SBIT(EX1, BYTE_IE, 2);              // enable external interrupt INT1
SBIT(ET0, BYTE_IE, 1);              // enable timer0 interrupt
SBIT(EX0, BYTE_IE, 0);              // enable external interrupt INT0

#define BYTE_IP          0xB8
SFR(IP, BYTE_IP);                   // interrupt priority and current priority
SBIT(PH_FLAG, BYTE_IP, 7);          // ReadOnly: high level priority action flag
SBIT(PL_FLAG, BYTE_IP, 6);          // ReadOnly: low level priority action flag
...
//    11 - unknown error
SBIT(PT2, BYTE_IP, 5);              // timer2 interrupt priority level
SBIT(PS, BYTE_IP, 4);               // UART0 interrupt priority level
SBIT(PT1, BYTE_IP, 3);              // timer1 interrupt priority level
SBIT(PX1, BYTE_IP, 2);              // external interrupt INT1 priority level
SBIT(PT0, BYTE_IP, 1);              // timer0 interrupt priority level
SBIT(PX0, BYTE_IP, 0);              // external interrupt INT0 priority level

#define BYTE_IE_EX       0xE8
SFR(IE_EX, BYTE_IE_EX);             // extend interrupt enable
SBIT(IE_WDOG, BYTE_IE_EX, 7);       // enable watch-dog timer interrupt
SBIT(IE_GPIO, BYTE_IE_EX, 6);       // enable GPIO input interrupt
SBIT(IE_PWMX, BYTE_IE_EX, 5);       // enable PWM1/2 interrupt
SBIT(IE_UART1, BYTE_IE_EX, 4);      // enable UART1 interrupt
SBIT(IE_ADC, BYTE_IE_EX, 3);        // enable ADC interrupt
SBIT(IE_USB, BYTE_IE_EX, 2);        // enable USB interrupt
SBIT(IE_TKEY, BYTE_IE_EX, 1);       // enable touch-key timer interrupt
SBIT(IE_SPI0, BYTE_IE_EX, 0);       // enable SPI0 interrupt
...
/*  Port Registers  */
#define BYTE_P1           0x90
SFR(P1, BYTE_P1);                   // port 1 input & output
SBIT(SCK, BYTE_P1, 7);              // serial clock for SPI0
SBIT(TXD1, BYTE_P1, 7);             // TXD output for UART1
SBIT(TIN5, BYTE_P1, 7);             // TIN5 for Touch-Key
SBIT(MISO, BYTE_P1, 6);             // master serial data input or slave serial data output for SPI0
SBIT(RXD1, BYTE_P1, 6);             // RXD input for UART1
SBIT(TIN4, BYTE_P1, 6);             // TIN4 for Touch-Key
SBIT(MOSI, BYTE_P1, 5);             // master serial data output or slave serial data input for SPI0
SBIT(PWM1, BYTE_P1, 5);             // PWM output for PWM1
SBIT(TIN3, BYTE_P1, 5);             // TIN3 for Touch-Key
SBIT(UCC2, BYTE_P1, 5);             // CC2 for USB type-C
SBIT(AIN2, BYTE_P1, 5);             // AIN2 for ADC
SBIT(T2_, BYTE_P1, 4);              // alternate pin for T2
SBIT(CAP1_, BYTE_P1, 4);            // alternate pin for CAP1
SBIT(SCS, BYTE_P1, 4);              // slave chip-selection input for SPI0
SBIT(TIN2, BYTE_P1, 4);             // TIN2 for Touch-Key
SBIT(UCC1, BYTE_P1, 4);             // CC1 for USB type-C
SBIT(AIN1, BYTE_P1, 4);             // AIN1 for ADC
SBIT(TXD_, BYTE_P1, 3);             // alternate pin for TXD of UART0
SBIT(RXD_, BYTE_P1, 2);             // alternate pin for RXD of UART0
SBIT(T2EX, BYTE_P1, 1);             // external trigger input for timer2 reload & capture
SBIT(CAP2, BYTE_P1, 1);             // capture2 input for timer2
SBIT(TIN1, BYTE_P1, 1);             // TIN1 for Touch-Key
SBIT(VBUS2, BYTE_P1, 1);            // VBUS2 for USB type-C
SBIT(AIN0, BYTE_P1, 1);             // AIN0 for ADC
SBIT(T2, BYTE_P1, 0);               // external count input
SBIT(CAP1, BYTE_P1, 0);             // capture1 input for timer2
SBIT(TIN0, BYTE_P1, 0);             // TIN0 for Touch-Key
...
#define BYTE_P3           0xB0
SFR(P3, BYTE_P3);                   // port 3 input & output
SBIT(UDM, BYTE_P3, 7);              // ReadOnly: pin UDM input
SBIT(UDP, BYTE_P3, 6);              // ReadOnly: pin UDP input
SBIT(T1, BYTE_P3, 5);               // external count input for timer1
SBIT(PWM2, BYTE_P3, 4);             // PWM output for PWM2
SBIT(RXD1_, BYTE_P3, 4);            // alternate pin for RXD1
SBIT(T0, BYTE_P3, 4);               // external count input for timer0
SBIT(INT1, BYTE_P3, 3);             // external interrupt 1 input
SBIT(TXD1_, BYTE_P3, 2);            // alternate pin for TXD1
SBIT(INT0, BYTE_P3, 2);             // external interrupt 0 input
SBIT(VBUS1, BYTE_P3, 2);            // VBUS1 for USB type-C
SBIT(AIN3, BYTE_P3, 2);             // AIN3 for ADC
SBIT(PWM2_, BYTE_P3, 1);            // alternate pin for PWM2
SBIT(TXD, BYTE_P3, 1);              // TXD output for UART0
SBIT(PWM1_, BYTE_P3, 0);            // alternate pin for PWM1
SBIT(RXD, BYTE_P3, 0);              // RXD input for UART0
...
/*  Timer0/1 Registers  */
#define BYTE_TCON         0x88
SFR(TCON, BYTE_TCON);               // timer 0/1 control and external interrupt control
SBIT(TF1, BYTE_TCON, 7);            // timer1 overflow & interrupt flag, auto cleared when MCU enter interrupt routine
SBIT(TR1, BYTE_TCON, 6);            // timer1 run enable
SBIT(TF0, BYTE_TCON, 5);            // timer0 overflow & interrupt flag, auto cleared when MCU enter interrupt routine
SBIT(TR0, BYTE_TCON, 4);            // timer0 run enable
SBIT(IE1, BYTE_TCON, 3);            // INT1 interrupt flag, auto cleared when MCU enter interrupt routine
SBIT(IT1, BYTE_TCON, 2);            // INT1 interrupt type: 0=low level action, 1=falling edge action
SBIT(IE0, BYTE_TCON, 1);            // INT0 interrupt flag, auto cleared when MCU enter interrupt routine
SBIT(IT0, BYTE_TCON, 0);            // INT0 interrupt type: 0=low level action, 1=falling edge action
...
/*  UART0 Registers  */
#define BYTE_SCON         0x98
SFR(SCON, BYTE_SCON);               // UART0 control (serial port control)
SBIT(SM0, BYTE_SCON, 7);            // UART0 mode bit0, selection data bit: 0=8 bits data, 1=9 bits data
SBIT(SM1, BYTE_SCON, 6);            // UART0 mode bit1, selection baud rate: 0=fixed, 1=variable
...
SBIT(SM2, BYTE_SCON, 5);            // enable multi-device communication in mode 2/3
#define MASK_UART0_MOD    0xE0      // bit mask of UART0 mode
SBIT(REN, BYTE_SCON, 4);            // enable UART0 receiving
SBIT(TB8, BYTE_SCON, 3);            // the 9th transmitted data bit in mode 2/3
SBIT(RB8, BYTE_SCON, 2);            // 9th data bit received in mode 2/3, or stop bit received for mode 1
SBIT(TI, BYTE_SCON, 1);             // transmit interrupt flag, set by hardware after completion of a serial transmittal, need software clear
SBIT(RI, BYTE_SCON, 0);             // receive interrupt flag, set by hardware after completion of a serial receiving, need software clear
SFR(SBUF, 0x99);                    // UART0 data buffer: reading for receiving, writing for transmittal

/*  Timer2/Capture2 Registers  */
#define BYTE_T2CON        0xC8
SFR(T2CON, BYTE_T2CON);             // timer 2 control
SBIT(TF2, BYTE_T2CON, 7);           // timer2 overflow & interrupt flag, need software clear, the flag will not be set when either RCLK=1 or TCLK=1
SBIT(CAP1F, BYTE_T2CON, 7);         // timer2 capture 1 interrupt flag, set by T2 edge trigger if bT2_CAP1_EN=1, need software clear
SBIT(EXF2, BYTE_T2CON, 6);          // timer2 external flag, set by T2EX edge trigger if EXEN2=1, need software clear
SBIT(RCLK, BYTE_T2CON, 5);          // selection UART0 receiving clock: 0=timer1 overflow pulse, 1=timer2 overflow pulse
SBIT(TCLK, BYTE_T2CON, 4);          // selection UART0 transmittal clock: 0=timer1 overflow pulse, 1=timer2 overflow pulse
SBIT(EXEN2, BYTE_T2CON, 3);         // enable T2EX trigger function: 0=ignore T2EX, 1=trigger reload or capture by T2EX edge
SBIT(TR2, BYTE_T2CON, 2);           // timer2 run enable
SBIT(C_T2, BYTE_T2CON, 1);          // timer2 clock source selection: 0=timer base internal clock, 1=external edge counter base T2 falling edge
SBIT(CP_RL2, BYTE_T2CON, 0);        // timer2 function selection (force 0 if RCLK=1 or TCLK=1): 0=timer and auto reload if count overflow or T2EX edge, 1=capture by T2EX edge
...
#define BYTE_SPI0_STAT    0xF8
SFR(SPI0_STAT, BYTE_SPI0_STAT);       // SPI 0 status
SBIT(S0_FST_ACT, BYTE_SPI0_STAT, 7);  // ReadOnly: indicate first byte received status for SPI0
SBIT(S0_IF_OV, BYTE_SPI0_STAT, 6);    // interrupt flag for slave mode FIFO overflow, direct bit address clear or write 1 to clear
SBIT(S0_IF_FIRST, BYTE_SPI0_STAT, 5); // interrupt flag for first byte received, direct bit address clear or write 1 to clear
SBIT(S0_IF_BYTE, BYTE_SPI0_STAT, 4);  // interrupt flag for a byte data exchanged, direct bit address clear or write 1 to clear or accessing FIFO to clear if bS0_AUTO_IF=1
SBIT(S0_FREE, BYTE_SPI0_STAT, 3);     // ReadOnly: SPI0 free status
SBIT(S0_T_FIFO, BYTE_SPI0_STAT, 2);   // ReadOnly: tx FIFO count for SPI0
SBIT(S0_R_FIFO, BYTE_SPI0_STAT, 0);   // ReadOnly: rx FIFO count for SPI0
...
/*  UART1 Registers  */
#define BYTE_SCON1        0xC0
SFR(SCON1, BYTE_SCON1);             // UART1 control (serial port control)
SBIT(U1SM0, BYTE_SCON1, 7);         // UART1 mode, selection data bit: 0=8 bits data, 1=9 bits data
SBIT(U1SMOD, BYTE_SCON1, 5);        // UART1 2X baud rate selection: 0=slow(Fsys/32/(256-SBAUD1)), 1=fast(Fsys/16/(256-SBAUD1))
SBIT(U1REN, BYTE_SCON1, 4);         // enable UART1 receiving
SBIT(U1TB8, BYTE_SCON1, 3);         // the 9th transmitted data bit in 9 bits data mode
SBIT(U1RB8, BYTE_SCON1, 2);         // 9th data bit received in 9 bits data mode, or stop bit received for 8 bits data mode
SBIT(U1TI, BYTE_SCON1, 1);          // transmit interrupt flag, set by hardware after completion of a serial transmittal, need software clear
SBIT(U1RI, BYTE_SCON1, 0);          // receive interrupt flag, set by hardware after completion of a serial receiving, need software clear
...
#define BYTE_ADC_CTRL     0x80
SFR(ADC_CTRL, BYTE_ADC_CTRL);       // ADC control
SBIT(CMPO, BYTE_ADC_CTRL, 7);       // ReadOnly: comparator result input
SBIT(CMP_IF, BYTE_ADC_CTRL, 6);     // flag for comparator result changed, direct bit address clear
SBIT(ADC_IF, BYTE_ADC_CTRL, 5);     // interrupt flag for ADC finished, direct bit address clear
SBIT(ADC_START, BYTE_ADC_CTRL, 4);  // set 1 to start ADC, auto cleared when ADC finished
SBIT(CMP_CHAN, BYTE_ADC_CTRL, 3);   // comparator IN- input channel selection: 0=AIN1, 1=AIN3
SBIT(ADC_CHAN1, BYTE_ADC_CTRL, 1);  // ADC/comparator IN+ channel selection high bit
SBIT(ADC_CHAN0, BYTE_ADC_CTRL, 0);  // ADC/comparator IN+ channel selection low bit
...
#define BYTE_USB_INT_FG   0xD8
SFR(USB_INT_FG, BYTE_USB_INT_FG);        // USB interrupt flag
SBIT(U_IS_NAK, BYTE_USB_INT_FG, 7);      // ReadOnly: indicate current USB transfer is NAK received
SBIT(U_TOG_OK, BYTE_USB_INT_FG, 6);      // ReadOnly: indicate current USB transfer toggle is OK
SBIT(U_SIE_FREE, BYTE_USB_INT_FG, 5);    // ReadOnly: indicate USB SIE free status
SBIT(UIF_FIFO_OV, BYTE_USB_INT_FG, 4);   // FIFO overflow interrupt flag for USB, direct bit address clear or write 1 to clear
SBIT(UIF_SUSPEND, BYTE_USB_INT_FG, 2);   // USB suspend or resume event interrupt flag, direct bit address clear or write 1 to clear
SBIT(UIF_TRANSFER, BYTE_USB_INT_FG, 1);  // USB transfer completion interrupt flag, direct bit address clear or write 1 to clear
SBIT(UIF_BUS_RST, BYTE_USB_INT_FG, 0);   // bus reset event interrupt flag, direct bit address clear or write 1 to clear
